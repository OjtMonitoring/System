    ...
    <div class="container">
        ...
        <h2 style="color:white; font-size:28px; padding:0 20px; position:absolute; left:18px; top: 200px;"> <?php echo $user_data['ojtname']; ?></h2>
            ...
            <div class="bottom-end">
                ...
                <div class="button">
                      <button><a href="supervisor-OjtStudents.php">OJT Students</a></button> 
...
exit();
  ...
  if(isset($_POST['submit'])){
    ...
    $username = $user_data['username'];
       ...
       if($new == $confirm){
        $query="SELECT * FROM supervisor WHERE password = '$old' and username = '$username'";
        $result=mysqli_query($conn,$query);
        $count=mysqli_num_rows($result);
        if($count>0){
            $query="UPDATE supervisor SET password = '$new' WHERE username = '$username' and password='$old'";
            $result=mysqli_query($conn,$query);
            echo '<div style="color:green;">Password change successfully</div>';
            echo '<div style="color:green;">Refresh this site</div>';
        }else{
           echo '<div style="color:red;">Old Password does not match</div>';
        }
    }else{
         echo '<div style="color:red;">New Password & Confirm Password does not match</div>';
    }
}
